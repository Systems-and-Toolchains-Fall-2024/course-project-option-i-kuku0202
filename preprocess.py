...
class Imputator(Transformer):
    ...
    def _transform(self, dataset):
        ...
        for col_name in df.columns:
            row_missing = df.filter(df[col_name].isNull()).count()
            if row_missing == 0:
                continue
                ...
                if (df.schema[col_name].dataType == FloatType()):
                    self.impute_dict[col_name] = df.filter(df[col_name].isNotNull()).select(mean(col_name)).collect()[0][0]
                elif (df.schema[col_name].dataType == IntegerType()):
                    self.impute_dict[col_name] = df.filter(df[col_name].isNotNull()).select(median(col_name)).collect()[0][0]
                elif (df.schema[col_name].dataType == StringType()):
                    self.impute_dict[col_name] = df.filter(df[col_name].isNotNull()).select(mode(col_name)).collect()[0][0]
                else:
...
def get_clearning_pipeline(df, col_to_drop):
    ...
    stage_imputator = Imputator(thresh=df.count()//5)
